...
def monitor_gi(): # récupérer le temps de réponse de la GI
    ...
    while monitoring:
        try:
            ...
            if response.status_code == 200:
                ...
                if pong_ts:
                    response_time = (pong_ts / 1000.0) - start_time # calculer le temps de réponse
                    
                    with lock: # verrouiller la liste des temps de réponse
...
@app.route('/start', methods=['POST']) # démarrer la surveillance
...
@app.route('/stop', methods=['POST']) # arrêter la surveillance
def stop_monitoring():
    global monitoring
    if monitoring:
        monitoring = False
...
@app.route('/data', methods=['GET']) # obtenir les données
def get_data():
