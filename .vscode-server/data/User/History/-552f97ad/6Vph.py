...
# Default queue for unknown source IPs
...
def send_to_s2():
    ...
    while send_running:
        for queue_name in priority_queues:
            if not priority_queues[queue_name].empty():
                pkt = priority_queues[queue_name].get()

                # Modify packet for destination
                pkt[IP].dst = "10.0.0.20"
                pkt[Ether].dst = "00:00:00:00:00:20"

                # Ensure the packet isn't oversized
                if len(bytes(pkt)) > 1500:  # Adjust based on MTU
                    print(f"Dropping oversized packet from {pkt[IP].src} (size: {len(bytes(pkt))})")
                    continue
        ...
        # Process packets in the default queue
        if not default_queue.empty():
            pkt = default_queue.get()
            print(f"Processing unknown packet from {pkt[IP].src}")
            # Add any specific handling for unknown packets if needed
