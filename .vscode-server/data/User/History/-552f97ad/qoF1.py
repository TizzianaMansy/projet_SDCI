...
vnf_interface = "eth0"  # Update with actual interface name

# Define the IP address of the switch (s2)
s2_ip = "10.0.0.12"  # Update with actual destination IP

# Function to process packets and prioritize based on source IP
def process_packet(pkt):
    if pkt.haslayer(IP):
        if pkt[IP].src == gf1_ip:
            priority_queues["gf1"].put(pkt)  # Highest priority
